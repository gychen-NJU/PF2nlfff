from ..needs import *
from ..extrapolation.nlfff_module import *
...
class PICNN():
    def __init__(self, PF_data, net=None, **kwargs):
        ...
        self.PF_data = PF_data if isinstance(PF_data, torch.Tensor) else torch.from_numpy(PF_data).float()
        self.device = kwargs.get('device',torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        self.net = (UNet3D().to(self.device) if net is None else net)
        ...
        self.print_epoch = 10
        self.save_epoch = 10000
        self._use_opt = True
        
        self.optimizer = torch.optim.AdamW(self.net.parameters(), lr=1.e-3, betas = (0.5, 0.999), weight_decay=1.e-3)
        self.scheduler = StepLR(self.optimizer, step_size=100, gamma=0.9)   
        
        self.save_models_dir = './trainer/3D_models/'
        self.save_model_name = 'netG'
        self.save_pics_dir = './trainer/3D_pics/'   
        
    def train(self, epoch=1000, lr=1e-3, Delta=1, gamma=0.994, **kwargs):
        save_best            = kwargs.get('save_best',False)
        save_first           = kwargs.get('save_first', False)
        opt_mt               = kwargs.get('opt_mt', 1.e-5)
        # self.save_models_dir = kwargs.get('save_models_dir', self.save_models_dir)
        # save_model_name      = kwargs.get('save_model_name', 'netG')
        attrs = ['print_epoch','save_epoch','save_models_dir', 'save_model_name']
        attrs+= ['L_div','L_ff','L_bc','L_sJ','L_fi','L_lf']
        for attr in attrs:
        ...
        PF_tensor = self.PF_data.unsqueeze(0).to(self.device).float()
        boundary = self.PF_data[...,0:1].clone().to(self.device).float()
        ...
        for iepoch in range(epoch):
            ...
            ff, div, sigma_J, fi, loss_lf = EQLoss(bcube)
            loss_lf = self.L_lf*loss_lf
            loss_div = self.L_div*div
            loss_ff = self.L_ff*ff
            loss_bc = self.L_bc*self.loss_func(boundary,bcube[:,:,:,0:1])
            self.optimizer.zero_grad()
            loss = loss_div+loss_ff+loss_bc+self.L_fi*fi+self.L_sJ*sigma_J+loss_lf
            loss.backward()
            self.loss = loss.detach().item()
            self.loss_list.append(self.loss)
            self.fi_list.append(fi.item())
            self.sJ_list.append(sJ.item())
            self.L_list.append(L0.item())
            self.optimizer.step()
            ...
            if self._use_opt:
                ...
                with torch.no_grad():
                    ...
                    mf_result = use_opt(bcube, boundary[:,:,:,0], dt=opt_mt)(is_print=False).to(self.device)
                    L1 = evaluate(mf_result)[2].detach()
                if L1<L0:
                    self.optimizer.zero_grad()
                    loss = self.loss_func(bcube,mf_result)
                    loss.backward()
                    self.optimizer.step()
                mf_result = mf_result.detach()
                del mf_result
                torch.cuda.empty_cache()
            self.scheduler.step()

            # print training information
            if (self.iter % self.print_epoch ==0) or first or (self.iter==epoch):
                time_now = time.time()
                time_used = time_now-start_time+self.wall_time
                print(
                'Iter %05d, Loss: %.3e, Loss_div: %.3e,Loss_ff: %.3e, Loss_bc: %.3e, Loss_lf: %.3e, wall_time: %.3e sec, lr: %.3e' % \
                    (self.iter, self.loss_list[-1], loss_div.item(), loss_ff.item(), loss_bc.item(), loss_lf.item(), time_used, 
                     self.scheduler.get_last_lr()[-1])
                )
            
            if self.iter % self.save_epoch == 0 or first or (self.iter==epoch):
                if not os.path.exists( self.save_models_dir):
                    os.makedirs( self.save_models_dir)
                    print('create the DIR. : %s' % self.save_models_dir)
                if (self.iter == 1) and save_first:
                    torch.save(self.net, os.path.join(self.save_models_dir,self.save_model_name+'_%05d.pkl') % self.iter)
                elif (self.iter !=1):
                    torch.save(self.net, os.path.join(self.save_models_dir,self.save_model_name+'_%05d.pkl') % self.iter)
                else:
                    pass
        ...
        torch.save(self.net, os.path.join(self.save_models_dir,self.save_model_name+'_%05d.pkl') % self.iter)
        if save_best:
            torch.save(best_model, os.path.join(self.save_models_dir,'best_model.pkl'))